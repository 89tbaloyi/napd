<?php
/**
 * ConsultStreet Customizer Notification Section Class.
 *
 * @package ConsultStreet
 */

/**
 * ConsultStreet_Customizer_Notice_Section class
 */
class ConsultStreet_Customizer_Notice_Section extends WP_Customize_Section {
	/**
	...
	public function json() {
		...
		foreach ( $consultstreet_recommended_plugins as $slug => $plugin_opt ) {
			...
			$consultstreet_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$consultstreet_recommended_plugin['class'] = 'active';
			} else {
				$consultstreet_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$consultstreet_recommended_plugin['button_class'] = 'install-now button';
					$consultstreet_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$consultstreet_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$consultstreet_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$consultstreet_recommended_plugin['button_class'] = 'deactivate-now button';
					$consultstreet_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$consultstreet_recommended_plugin['id'] = $slug;
			$consultstreet_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$consultstreet_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$consultstreet_recommended_plugin['description'] = $info->short_description;
			}

			$consultstreet_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $consultstreet_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="consultstreet-recommeded-actions-container consultstreet-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="consultstreet-recommeded-actions">
